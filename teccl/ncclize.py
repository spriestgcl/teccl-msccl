...
def _remap_scratch_into_input_output(liveness, gpus, logging):
    ...
    '''
    if not HAS_Z3:
        raise ImportError('z3 is required for scratch remapping but is not installed.')
...
def _allocate_channels_max_concurrency(op_sets, logging):
    ...
    if not HAS_Z3:
        raise ImportError('z3 is required for ChannelPolicy.MaxConcurrency but is not installed.')
...
def _allocate_channels_address_two(op_sets):
    ops_by_channel = defaultdict(list)
    for op_set in op_sets:
        send = op_set[0]
        assert send.op_type == 's'
        base_off = send.src_offset if send.src_offset is not None else 0
        chan = base_off % 2
        ops_by_channel[chan].extend(op_set)
    return dict(ops_by_channel)
...
def ncclize(algorithm, remap_scratch = None, channel_policy=ChannelPolicy.MatchTopology, pretty_print = True, old_format=False, use_scratch=False, merge_contiguous=True, instances=1, scale_remote=1, combine_contig=False, aid_IB_contig=False, prefix="", logging=False, epoch_tb_grouping=False, include_algostep=True):
    '''
    ...
    elif channel_policy == ChannelPolicy.AddressTwo:
        ops_by_channel = _allocate_channels_address_two(op_sets)
    ...
    # Greedily build threadblocks per GPU/channel. A TB is keyed by the pair of
    # communication peers it serves on that GPU rather than by epoch.
    for chan, chan_ops in ops_by_channel.items():
        chan_ops = sorted(
            ...
            key=lambda op: (
                op.gpu,
                op.step,
                0 if op.op_type == 'r' else 1 if op.op_type == 's' else 2,
                op.src_offset if op.src_offset is not None else -1,
                op.dst_offset if op.dst_offset is not None else -1,
                op.peer if op.peer is not None else -1,
            ),
        )
        for op in chan_ops:
            rank = op.gpu
            ...
            for tb in tbs:
                send_ok = tb.send == -1 or (op.is_send and tb.send == op.peer) or (not op.is_send)
                recv_ok = tb.recv == -1 or ((not op.is_send) and tb.recv == op.peer) or op.is_send
                ...
                tbs.append(tb)
            if op.is_send:
                assert tb.send == -1 or tb.send == op.peer
                tb.send = op.peer
            else:
                assert tb.recv == -1 or tb.recv == op.peer
                tb.recv = op.peer
            tb.steps.append(op)
    ...
    for rank, gpu in gpus.items():
        ...
        for tb in gpu.threadblocks:
            ...
            for op in tb.ops:
                ...
                if include_algostep:
                    op_elem.set('algostep', str(op.step))
